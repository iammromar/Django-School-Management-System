...
from student_management_app.models import CustomUser, Staffs, Group, Subjects, Students, Attendance, AttendanceReport, LeaveReportStudent, FeedBackStudent, StudentResult
...
def student_home(request):
    ...
    data_present = []
    data_absent = []
    group_data = student_obj.group_id.all() #changed
    for group in group_data:
        attendance = Attendance.objects.filter(group_id=group.id)
...
def student_view_attendance(request):
    ...
    groups = student.group_id.all() # Getting the Subjects of group Enrolled
    context = {
        "groups": groups
    }
...
def student_view_attendance_post(request):
    if request.method != "POST":
        ...
        attendance = Attendance.objects.filter(attendance_date__range=(start_date_parse, end_date_parse), group_id=group_obj)
